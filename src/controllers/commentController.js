    ...
    if (user) {
      ...
      return res
        ...
        .json({
          ...
          "List Comment": question.comments,
        });
    }
    question = await Question.findById(id).populate("comments");
    return res
      .status(200)
      .json({ message: "Comment success!", "List Comment": question.comments });
  } catch (err) {
    console.log(err);
    return res
      .status(500)
      .json({ message: "Something is wrong!", err: { err } });
  }
};
...
exports.getComment = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Getting success!", "List Comment": question.comments });
  } catch (err) {
    console.log(err);
    return res
      .status(500)
      .json({ message: "Something is wrong!", err: err.messages });
  }
};
...
exports.replyComment = async (req, res) => {
  ...
  try {
    ...
    if (commentOwner._id.toString() != user._id.toString()) {
      ...
      const typeNotice = `${comment.Post_ID}`;
      const newNotice = new Notification({
    ...
    return res
      .status(200)
      .json({ message: "replying success!", Comment: getComment });
  } catch (error) {
    console.log(error);
...
exports.getReplyComment = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Getting success!", "Reply Comment": comment });
  } catch (error) {
    console.log(error);
...
exports.voteComment = async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      if (!vote) {
        ...
        if (commentOwner.idRank == false) {
          console.log("Chưa có rank");
          const newRank = await new Rank({
            uid: commentOwner._id,
            username: commentOwner.name,
            avt: commentOwner.avatar,
          });
        ...
        if (commentOwner._id.toString() != user._id.toString()) {
          ...
          let typeNotice = `${comment.Post_ID}`;
          ...
          return res
            .status(403)
            .json({
              message: "Bạn không thể đánh giá bình luận của chính mình",
            });
        }
        const commentVoted = await Vote.find({Comment_ID: comment._id});
        return res
          .status(200)
          .json({ message: "Voting success!", Comment: comment, votes: commentVoted });
      } else {
        return res
          .status(403)
          .json({
            message:
              "Bạn đã vote trước đó rồi, nếu muốn vote lại vui lòng gỡ bình chọn cũ",
          });
      }
    }
    if (!comment) {
    ...
    return res
      .status(500)
      .json({ message: "Vote failed", error: error.messages });
  }
};
...
exports.getVote = async (req, res) => {
  const idComment = req.params.idCmt;
  try {
    let vote = await Vote.find({Comment_ID: idComment});
    return res
      .status(200)
      .json({ message: "Get voting success!", "List Vote": vote});
  } catch (err) {
    console.log(err);
...
exports.getOneVote = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Voting success!", "List Vote": vote });
  } catch (err) {
    console.log(err);
    return res.status(500).json({ message: "Something is wrong!", err });
  }
...
exports.deleteVote = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({
        message: "Delete vote is successful!",
        quantity: count,
        "List Vote": vote,
      });
  } catch (err) {
